# remotyy Configuration Reference

## Config File Locations

remotyy looks for config in this order:
1. CLI flag: `--config /path/to/remotyy.yaml`
2. `./remotyy.yaml`
3. `~/.remotyy/remotyy.yaml`
4. `/etc/remotyy/remotyy.yaml`

## Global

| Field | Type | Default | Env | Description |
|-------|------|---------|-----|-------------|
| `global.data_dir` | string | `~/.remotyy` | — | Data storage directory |
...
| `signal.auth_token` | string | `""` | `REMOTYY_AUTH_TOKEN` | Auth token for WS connections. Empty = no auth. |
...
| `signal.tls.cert_file` | string | `""` | `REMOTYY_TLS_CERT` | TLS certificate path |
| `signal.tls.key_file` | string | `""` | `REMOTYY_TLS_KEY` | TLS key path |
...
| Field | Type | Default | Env | Description |
|-------|------|---------|-----|-------------|
| `host.signal_url` | string | — | `REMOTYY_SIGNAL_URL` | Signal server URL (e.g. `ws://localhost:9000`) |
| `host.name` | string | hostname | `REMOTYY_HOST_NAME` | Display name for this host |
| `host.master_password` | string | `""` | `REMOTYY_MASTER_PASSWORD` | Plaintext password (auto-hashed on start) |
...
| `host.device_id` | string | — | `REMOTYY_DEVICE_ID` | Stable device identifier |
...
| Field | Type | Default | Env | Description |
|-------|------|---------|-----|-------------|
| `client.signal_url` | string | — | `REMOTYY_SIGNAL_URL` | Signal server URL |
| `client.host_id` | string | — | — | Target host ID or name |
| `client.master_password` | string | `""` | `REMOTYY_MASTER_PASSWORD` | Password for authentication |
...
| Field | Type | Default | Env | Description |
|-------|------|---------|-----|-------------|
| `logging.level` | string | `info` | `REMOTYY_LOG_LEVEL` | debug, info, warn, error, trace, fatal, disabled |
| `logging.format` | string | `console` | — | Output format: console, json |
| `logging.file` | string | `""` | `REMOTYY_LOG_FILE` | Log file path. Empty = stderr. |
...
```yaml
# remotyy.yaml — Production configuration

global:
  data_dir: /var/lib/remotyy
...
signal:
  ...
  tls:
    enabled: true
    cert_file: /etc/ssl/certs/remotyy.pem
    key_file: /etc/ssl/private/remotyy.key

host:
  signal_url: wss://remotyy.example.com:9000
  name: "my-server"
